...
public class ShadowfallModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, ShadowfallMod.MODID);
	public static final RegistryObject<Biome> SHADELANDS = REGISTRY.register("shadelands", () -> ShadelandsBiome.createBiome());
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(ShadelandsBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, SHADELANDS.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1,
									preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, SHADELANDS.getId()),
											Blocks.GRASS_BLOCK.defaultBlockState(), ShadowfallModBlocks.SHADE_SHALE.get().defaultBlockState(),
											ShadowfallModBlocks.SHADE_SHALE.get().defaultBlockState()));
